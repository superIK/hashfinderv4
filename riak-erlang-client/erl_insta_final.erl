-module(erl_insta_final).
 -compile(export_all).
 start(Pid) -> ssl:start(), application:start(inets), retrieve(Pid).
 ...
 retrieve (Pid) ->
  ...
  case X of
    {ok,{_,_,JSON}}->
      ...
      Final_List=remove_commas(Parsed_List),Final_List, connect(Final_List, Pid);
    _-> retrieve(Pid)end.

 connect(List, Pid) ->
  store_data(List, Pid),
  stored. store_data([],Pid) -> all_saved;
...
final_list(List) -> final_list(List,[]).
 final_list([],Acc) -> Acc; final_list([[{"tags",{array,L}},{"id",V}]|T],Acc) -> final_list(T, Acc ++ [{V,L}]). is_convertable(X)->
  try
    Y = list_to_binary(X),
    true
  catch
 ...
 parse(S) -> {struct, List} = S, A=proplists:delete("filter",List), B=proplists:delete("link", A),B, C=proplists:delete("attribution", B), D=proplists:delete("type",C), 
E=proplists:delete("comments",D), F=proplists:delete("users_in_photo", E), G=proplists:delete("caption",F), H=proplists:delete("likes",G), I=proplists:delete("images",H), 
J=proplists:delete("user_has_liked",I), K=proplists:delete("user",J), L=proplists:delete("videos",K), M=proplists:delete("location",L), N=proplists:delete("created_time",M),N.
